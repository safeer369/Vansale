...
class SalesReturnActivity : AppCompatActivity() {
    ...
    var listInvoices = ArrayList<SalesReturnInvoiceModel>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        listInvoices = DataCollections.getInstance(this).getSalesReturnInvoices()
        listInvoices.add(SalesReturnInvoiceModel("", "", TypeManager.INVOICE_TYPE_FRESH))

        vp.adapter = SalesReturnVpAdapter(supportFragmentManager, listInvoices)
...
//            val salesFragment = supportFragmentManager.findFragmentByTag("android:switcher:" + R.id.vp + ":" + vp.currentItem) as SalesFragment
            val salesFragment = (vp.adapter as SalesReturnVpAdapter).currentFragment!!
            if (salesFragment.productItemsList.size > 0 && salesFragment.invoiceStatus == TypeManager.INVOICE_TYPE_FRESH) {
                AlertDialog.Builder(this)
                        ...
                        .setPositiveButton("YES") { p0, p1 ->
                            isExit = true
                            this@SalesReturnActivity.onBackPressed()
                        }
