...
class PurchaseFinishActivity : AppCompatActivity() {
    ...
    private lateinit var tvValueExpense: TextView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_purchase_finish)
        init()
        ...
        fabSave.setOnClickListener {
            ...
            var expense = tvValueExpense.text.toString()
            ...
            expense = ("%." + DataCollections.getInstance(this).getUser()!!.amountType + "f").format(MathUtils.toDouble(expense))
                        ...
                        .setPositiveButton("YES") { p0, p1 ->
                            if (bt.isTurning() || dbp.isDiscovering() || (printTask != null && printTask!!.status == AsyncTask.Status.RUNNING)) {
                                AlertDialog.Builder(this)
                                        ...
                                        .setPositiveButton("YES") { p0, p1 ->
                                            if (bt.isTurning())
                                                bt.cancelListener(this@PurchaseFinishActivity)
            ...
            override fun onFailure(call: Call?, e: IOException?) {
                ...
                runOnUiThread {
                    ...
                    if (e is UnknownHostException)
                        SnackBarUtils.showSnackBar(this@PurchaseFinishActivity, getString(R.string.error_message_connect_error))
                    else
                        SnackBarUtils.showSnackBar(this@PurchaseFinishActivity, getString(R.string.error_message_went_wront))
                }
            }
                ...
                try {
                    ...
                    if (result) {
                        ...
                        runOnUiThread {
                            ...
                            SnackBarUtils.showSnackBar(this@PurchaseFinishActivity, message)
                        }
                    } else {
                        val message = jo.getString("Message")
                        runOnUiThread {
                    ...
                    runOnUiThread {
                        pbLoadingSaveAsPDF.visibility = View.GONE
                        fabSaveAsPDF.isEnabled = true
                        SnackBarUtils.showSnackBar(this@PurchaseFinishActivity, "$resp")
                    }
                }
            }
        })
    ...
    private fun uploadData(isUpdate: Boolean,
        ...
        joSalesSummery.put("SupplierID", customerId)
        joSalesSummery.put("PurchaseDate", date)
        joSalesSummery.put("SupplierName", name)
        joSalesSummery.put("SupplierAddress", address)
        ...
        joSalesSummery.put("ExpenseAmount", expense)
        ...
        Log.e("_______________req", "save invoice purchase summery :" + jaSalesSummery.toString())
        Log.e("_______________req", "save invoice purchase details :" + jaSalesDetails.toString())

        val body = FormBody.Builder()
                .add("UserID", DataCollections.getInstance(this).getUser()?.id ?: "")
                .add("PurchaseSummary", jaSalesSummery.toString())
                .add("PurchaseDetails", jaSalesDetails.toString())

        if (isUpdate) {
            body.add("PurchaseID", invoiceId)
        }

        val url = "https://api.myjson.com/bins/1elzb0"
        val request = Request.Builder()
                .url(if (isUpdate) PublicUrls.URL_EDIT_PURCHASE_INVOICE else PublicUrls.URL_SAVE_PURCHASE_INVOICE)
                .post(body.build())
                .build()
                ...
                try {
                    ...
                    if (result) {
                        ...
                        runOnUiThread {
                            ...
                            if (!isUpdate)
                                DataCollections.getInstance(this@PurchaseFinishActivity).addPurchaseInvoice(ivId, ivNo, TypeManager.INVOICE_TYPE_OLD)
                            SnackBarUtils.showSnackBar(this@PurchaseFinishActivity, message)
    ...
    private val bt = BluetoothUtils(object : BluetoothUtils.BluetoothListener {
        override fun onSuccess(isOn: Boolean) {
            if (isOn) {
                dbp.discover(this@PurchaseFinishActivity)
            }
        }
        ...
        override fun onError() {
            ...
            SnackBarUtils.showSnackBar(this@PurchaseFinishActivity, "Cant turn on bluetooth")
        }
    })
    ...
    private val dbp = BluetoothPrinterDiscovery(object : BluetoothPrinterDiscovery.PrinterDiscoverListener {
        override fun onSuccess(discoveredPrinter: DiscoveredPrinter) {
            ...
            try {
                ...
                if (zebraPrinter.currentStatus.isReadyToPrint || zebraPrinter.currentStatus.isPaused) {
                    val header = StringUtils.derialize(DataCollections.getInstance(this@PurchaseFinishActivity).getUser()!!.header)
                    val footer = StringUtils.derialize(DataCollections.getInstance(this@PurchaseFinishActivity).getUser()!!.footer)
                    val cash = ("%." + DataCollections.getInstance(this@PurchaseFinishActivity).getUser()!!.amountType + "f").format(MathUtils.toDouble(tvValueCash.text.toString()))
                    val card = ("%." + DataCollections.getInstance(this@PurchaseFinishActivity).getUser()!!.amountType + "f").format(MathUtils.toDouble(tvValueCredit.text.toString()))
                    val balanceAmt = tvValueBalance.text.toString()
                    val discountAmt =  ("%." + DataCollections.getInstance(this@PurchaseFinishActivity).getUser()!!.amountType + "f").format(MathUtils.toDouble(tvValueDiscountAmount.text.toString()))
                    val amtType = DataCollections.getInstance(this@PurchaseFinishActivity).getUser()!!.amountType
                ...
                fabPrint.isEnabled = true
                pbLoadingPrint.visibility = View.GONE
                SnackBarUtils.showSnackBar(this@PurchaseFinishActivity, "Can't connect to printer")
            }
        }
        ...
        override fun onFail() {
            fabPrint.isEnabled = true
            pbLoadingPrint.visibility = View.GONE
            SnackBarUtils.showSnackBar(this@PurchaseFinishActivity, "Printer not found")
        }
    })
    ...
    private val printResult = object : PrintTask.ZebraPrinterNotify {
        override fun notifyChange() {
            fabPrint.isEnabled = true
            pbLoadingPrint.visibility = View.GONE
            SnackBarUtils.showSnackBar(this@PurchaseFinishActivity, "Print success")
        }
    }
                        ...
                        .setPositiveButton("YES") { p0, p1 ->
                            if (bt.isTurning() || dbp.isDiscovering() || (printTask != null && printTask!!.status == AsyncTask.Status.RUNNING)) {
                                AlertDialog.Builder(this)
                                        ...
                                        .setPositiveButton("YES") { p0, p1 ->
                                            isExit = true
                                            this@PurchaseFinishActivity.onBackPressed()
                                            if (bt.isTurning())
                                                bt.cancelListener(this@PurchaseFinishActivity)
                                            if (dbp.isDiscovering())
                                                dbp.cancelDiscovery()
    ...
    private fun init() {
        ...
        tvValueExpense = findViewById(R.id.tvValueExpense)
    }
