...
class PurchaseReturnActivity : AppCompatActivity() {
    ...
    var listInvoices = ArrayList<PurchaseReturnInvoiceModel>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        listInvoices = DataCollections.getInstance(this).getPurchaseReturnInvoices()
        listInvoices.add(PurchaseReturnInvoiceModel("", "", TypeManager.INVOICE_TYPE_FRESH))

        vp.adapter = PurchaseReturnVpAdapter(supportFragmentManager, listInvoices)
...
//            val salesFragment = supportFragmentManager.findFragmentByTag("android:switcher:" + R.id.vp + ":" + vp.currentItem) as SalesFragment
            val salesFragment = (vp.adapter as PurchaseReturnVpAdapter).currentFragment!!
            if (salesFragment.productItemsList.size > 0 && salesFragment.invoiceStatus == TypeManager.INVOICE_TYPE_FRESH) {
                AlertDialog.Builder(this)
                        ...
                        .setPositiveButton("YES") { p0, p1 ->
                            isExit = true
                            this@PurchaseReturnActivity.onBackPressed()
                        }
