...
import com.teamayka.vansaleandmgmt.ui.filedforce.adapters.RvInProgressListAdapter
...
class InProgressActivity : AppCompatActivity() {
    ...
    inner class TouchHelper(private val adapter: RvInProgressListAdapter) : ItemTouchHelper.SimpleCallback(ItemTouchHelper.UP or ItemTouchHelper.DOWN, 0) {
        private var vh: RvInProgressListAdapter.ViewHolder? = null

        override fun onSelectedChanged(viewHolder: RecyclerView.ViewHolder?, actionState: Int) {
            super.onSelectedChanged(viewHolder, actionState)
            if (viewHolder is RvInProgressListAdapter.ViewHolder) {
                vh = viewHolder
                viewHolder.layoutParent.setBackgroundColor(if (actionState == ItemTouchHelper.ACTION_STATE_DRAG) Color.parseColor("#dbdbdb") else {
                    Handler().postDelayed({
                        try {
                            ...
                            if (rvList.adapter is RvInProgressListAdapter) {
                                val list = (rvList.adapter as RvInProgressListAdapter).list
                                DataCollections.getInstance(this@InProgressActivity).updateInProgressList(list, date)
                            }
                        } catch (e: Exception) {
                        }
                    }, 500)
    ...
    private fun getReports() {
        ...
        val body = FormBody.Builder()
                ...
                .add("Date", date)
                .add("Type", FieldForceTypes.IN_PROGRESS)
                .build()

        Log.e("__________post", "_field_force_in_progress : ")
        ...
        callGetList?.enqueue(object : Callback {
            ...
            override fun onResponse(call: Call?, response: Response?) {
                ...
                Log.e("_____________resp", "_field_force_in_progress : $resp")
                try {
                    ...
                    Log.e("_______exc", "_field_force_in_progress : " + e.message)
                    runOnUiThread {
                        rvList.visibility = View.GONE
                        ivMap.visibility = View.GONE
                        ivRefresh.visibility = View.GONE
    ...
    private fun showList(list: ArrayList<RvListModel>) {
        ...
        if (date == CalendarUtils.getCurrentDate())
            previousList = DataCollections.getInstance(this).getInProgressList()
        ...
        if (rvList.adapter is RvInProgressListAdapter) {
            (rvList.adapter as RvInProgressListAdapter).updateList(list)
            DataCollections.getInstance(this).updateInProgressList(list, date)
        } else {
            rvList.adapter = RvInProgressListAdapter(previousList, date)
            val callBack = TouchHelper(rvList.adapter as RvInProgressListAdapter)
        ...
        ivMap.setOnClickListener {
            ...
            intent.putParcelableArrayListExtra("KEY_LIST", (rvList.adapter as RvInProgressListAdapter).list)
        ...
        ivRefresh.setOnClickListener {
            AlertDialog.Builder(this)
                    ...
                    .setPositiveButton("OPTIMISE ROUTE", DialogInterface.OnClickListener { p0, p1 ->
                        val location = PreferenceManager.getDefaultSharedPreferences(this@InProgressActivity).getString("KEY_CURRENT_LOCATION", "")
                        if (TextUtils.isEmpty(location)) {
                            SnackBarUtils.showSnackBar(this@InProgressActivity, "Can't get location")
