...
class SalesOrderFinishActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_sales_order_finish)
        init()
...
//            }
                        ...
                        .setPositiveButton("YES") { p0, p1 ->
                            if (bt.isTurning() || dbp.isDiscovering() || (printTask != null && printTask!!.status == AsyncTask.Status.RUNNING)) {
                                AlertDialog.Builder(this)
                                        ...
                                        .setPositiveButton("YES") { p0, p1 ->
                                            if (bt.isTurning())
                                                bt.cancelListener(this@SalesOrderFinishActivity)
            ...
            override fun onFailure(call: Call?, e: IOException?) {
                ...
                runOnUiThread {
                    ...
                    if (e is UnknownHostException)
                        SnackBarUtils.showSnackBar(this@SalesOrderFinishActivity, getString(R.string.error_message_connect_error))
                    else
                        SnackBarUtils.showSnackBar(this@SalesOrderFinishActivity, getString(R.string.error_message_went_wront))
                }
            }
                ...
                try {
                    ...
                    if (result) {
                        ...
                        runOnUiThread {
                            ...
                            SnackBarUtils.showSnackBar(this@SalesOrderFinishActivity, message)
                        }
                    } else {
                        val message = jo.getString("Message")
                        runOnUiThread {
                    ...
                    runOnUiThread {
                        pbLoadingSaveAsPDF.visibility = View.GONE
                        fabSaveAsPDF.isEnabled = true
                        SnackBarUtils.showSnackBar(this@SalesOrderFinishActivity, "$resp")
                    }
                }
            }
        })
    ...
    private fun uploadData(isUpdate: Boolean,
        ...
        Log.e("_______________req", "save invoice sales summery :" + jaSalesSummery.toString())
        Log.e("_______________req", "save invoice sales details :" + jaSalesDetails.toString())
        ...
        val request = Request.Builder()
                .url(if (isUpdate) PublicUrls.URL_EDIT_SALES_ORDER_INVOICE else PublicUrls.URL_SAVE_SALES_ORDER_INVOICE)
                .post(body.build())
                .build()
                ...
                try {
                    ...
                    if (result) {
                        ...
                        runOnUiThread {
                            ...
                            if (!isUpdate)
                                DataCollections.getInstance(this@SalesOrderFinishActivity).addSalesOrderInvoice(ivId, ivNo, TypeManager.INVOICE_TYPE_OLD)
                            SnackBarUtils.showSnackBar(this@SalesOrderFinishActivity, message)
    ...
    private val bt = BluetoothUtils(object : BluetoothUtils.BluetoothListener {
        override fun onSuccess(isOn: Boolean) {
            if (isOn) {
                dbp.discover(this@SalesOrderFinishActivity)
            }
        }
        ...
        override fun onError() {
            ...
            SnackBarUtils.showSnackBar(this@SalesOrderFinishActivity, "Cant turn on bluetooth")
        }
    })
    ...
    private val dbp = BluetoothPrinterDiscovery(object : BluetoothPrinterDiscovery.PrinterDiscoverListener {
        override fun onSuccess(discoveredPrinter: DiscoveredPrinter) {
            ...
            try {
                ...
                if (zebraPrinter.currentStatus.isReadyToPrint || zebraPrinter.currentStatus.isPaused) {
                    val header = StringUtils.derialize(DataCollections.getInstance(this@SalesOrderFinishActivity).getUser()!!.header)
                    val footer = StringUtils.derialize(DataCollections.getInstance(this@SalesOrderFinishActivity).getUser()!!.footer)
                    val cash = ("%." + DataCollections.getInstance(this@SalesOrderFinishActivity).getUser()!!.amountType + "f").format(MathUtils.toDouble(tvValueCash.text.toString()))
                    val card = ("%." + DataCollections.getInstance(this@SalesOrderFinishActivity).getUser()!!.amountType + "f").format(MathUtils.toDouble(tvValueCredit.text.toString()))
                    val balanceAmt = tvValueBalance.text.toString()
                    val discountAmt =  ("%." + DataCollections.getInstance(this@SalesOrderFinishActivity).getUser()!!.amountType + "f").format(MathUtils.toDouble(tvValueDiscountAmount.text.toString()))
                    val amtType = DataCollections.getInstance(this@SalesOrderFinishActivity).getUser()!!.amountType
                ...
                fabPrint.isEnabled = true
                pbLoadingPrint.visibility = View.GONE
                SnackBarUtils.showSnackBar(this@SalesOrderFinishActivity, "Can't connect to printer")
            }
        }
        ...
        override fun onFail() {
            fabPrint.isEnabled = true
            pbLoadingPrint.visibility = View.GONE
            SnackBarUtils.showSnackBar(this@SalesOrderFinishActivity, "Printer not found")
        }
    })
    ...
    private val printResult = object : PrintTask.ZebraPrinterNotify {
        override fun notifyChange() {
            fabPrint.isEnabled = true
            pbLoadingPrint.visibility = View.GONE
            SnackBarUtils.showSnackBar(this@SalesOrderFinishActivity, "Print success")
        }
    }
                        ...
                        .setPositiveButton("YES") { p0, p1 ->
                            if (bt.isTurning() || dbp.isDiscovering() || (printTask != null && printTask!!.status == AsyncTask.Status.RUNNING)) {
                                AlertDialog.Builder(this)
                                        ...
                                        .setPositiveButton("YES") { p0, p1 ->
                                            isExit = true
                                            this@SalesOrderFinishActivity.onBackPressed()
                                            if (bt.isTurning())
                                                bt.cancelListener(this@SalesOrderFinishActivity)
                                            if (dbp.isDiscovering())
                                                dbp.cancelDiscovery()
