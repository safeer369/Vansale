...
class SalesFinishActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_sales_finish)
        init()
                        ...
                        .setPositiveButton("YES") { p0, p1 ->
                            if (bt.isTurning() || dbp.isDiscovering() || (printTask != null && printTask!!.status == AsyncTask.Status.RUNNING)) {
                                AlertDialog.Builder(this)
                                        ...
                                        .setPositiveButton("YES") { p0, p1 ->
                                            if (bt.isTurning())
                                                bt.cancelListener(this@SalesFinishActivity)
            ...
            override fun onFailure(call: Call?, e: IOException?) {
                ...
                runOnUiThread {
                    ...
                    if (e is UnknownHostException)
                        SnackBarUtils.showSnackBar(this@SalesFinishActivity, getString(R.string.error_message_connect_error))
                    else
                        SnackBarUtils.showSnackBar(this@SalesFinishActivity, getString(R.string.error_message_went_wront))
                }
            }
                ...
                try {
                    ...
                    if (result) {
                        ...
                        runOnUiThread {
                            ...
                            SnackBarUtils.showSnackBar(this@SalesFinishActivity, message)
                        }
                    } else {
                        val message = jo.getString("Message")
                        runOnUiThread {
                    ...
                    runOnUiThread {
                        pbLoadingSaveAsPDF.visibility = View.GONE
                        fabSaveAsPDF.isEnabled = true
                        SnackBarUtils.showSnackBar(this@SalesFinishActivity, "$resp")
                    }
                }
            }
        })
    ...
    private fun uploadData(isUpdate: Boolean,
        ...
        val request = Request.Builder()
                .url(if (isUpdate) PublicUrls.URL_EDIT_SALES_INVOICE else PublicUrls.URL_SAVE_SALES_INVOICE)
                .post(body.build())
                .build()
                ...
                try {
                    ...
                    if (result) {
                        ...
                        runOnUiThread {
                            ...
                            if (!isUpdate)
                                DataCollections.getInstance(this@SalesFinishActivity).addSalesInvoice(ivId, ivNo, TypeManager.INVOICE_TYPE_OLD)
                            SnackBarUtils.showSnackBar(this@SalesFinishActivity, message)
    ...
    private val bt = BluetoothUtils(object : BluetoothUtils.BluetoothListener {
        override fun onSuccess(isOn: Boolean) {
            if (isOn) {
                dbp.discover(this@SalesFinishActivity)
            }
        }
        ...
        override fun onError() {
            ...
            SnackBarUtils.showSnackBar(this@SalesFinishActivity, "Cant turn on bluetooth")
        }
    })
    ...
    private val dbp = BluetoothPrinterDiscovery(object : BluetoothPrinterDiscovery.PrinterDiscoverListener {
        override fun onSuccess(discoveredPrinter: DiscoveredPrinter) {
            ...
            try {
                ...
                if (zebraPrinter.currentStatus.isReadyToPrint || zebraPrinter.currentStatus.isPaused) {
                    val header = StringUtils.derialize(DataCollections.getInstance(this@SalesFinishActivity).getUser()!!.header)
                    val footer = StringUtils.derialize(DataCollections.getInstance(this@SalesFinishActivity).getUser()!!.footer)
                    val cash = ("%." + DataCollections.getInstance(this@SalesFinishActivity).getUser()!!.amountType + "f").format(MathUtils.toDouble(tvValueCash.text.toString()))
                    val card = ("%." + DataCollections.getInstance(this@SalesFinishActivity).getUser()!!.amountType + "f").format(MathUtils.toDouble(tvValueCredit.text.toString()))
                    val balanceAmt = tvValueBalance.text.toString()
                    val discountAmt =  ("%." + DataCollections.getInstance(this@SalesFinishActivity).getUser()!!.amountType + "f").format(MathUtils.toDouble(tvValueDiscountAmount.text.toString()))
                    val amtType = DataCollections.getInstance(this@SalesFinishActivity).getUser()!!.amountType
                ...
                fabPrint.isEnabled = true
                pbLoadingPrint.visibility = View.GONE
                SnackBarUtils.showSnackBar(this@SalesFinishActivity, "Can't connect to printer")
            }
        }
        ...
        override fun onFail() {
            fabPrint.isEnabled = true
            pbLoadingPrint.visibility = View.GONE
            SnackBarUtils.showSnackBar(this@SalesFinishActivity, "Printer not found")
        }
    })
    ...
    private val printResult = object : PrintTask.ZebraPrinterNotify {
        override fun notifyChange() {
            fabPrint.isEnabled = true
            pbLoadingPrint.visibility = View.GONE
            SnackBarUtils.showSnackBar(this@SalesFinishActivity, "Print success")
        }
    }
                        ...
                        .setPositiveButton("YES") { p0, p1 ->
                            if (bt.isTurning() || dbp.isDiscovering() || (printTask != null && printTask!!.status == AsyncTask.Status.RUNNING)) {
                                AlertDialog.Builder(this)
                                        ...
                                        .setPositiveButton("YES") { p0, p1 ->
                                            isExit = true
                                            this@SalesFinishActivity.onBackPressed()
                                            if (bt.isTurning())
                                                bt.cancelListener(this@SalesFinishActivity)
                                            if (dbp.isDiscovering())
                                                dbp.cancelDiscovery()
