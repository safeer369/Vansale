...
class PurchaseReturnFinishActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_purchase_return_finish)
        init()
                        ...
                        .setPositiveButton("YES") { p0, p1 ->
                            if (bt.isTurning() || dbp.isDiscovering() || (printTask != null && printTask!!.status == AsyncTask.Status.RUNNING)) {
                                AlertDialog.Builder(this)
                                        ...
                                        .setPositiveButton("YES") { p0, p1 ->
                                            if (bt.isTurning())
                                                bt.cancelListener(this@PurchaseReturnFinishActivity)
            ...
            override fun onFailure(call: Call?, e: IOException?) {
                ...
                runOnUiThread {
                    ...
                    if (e is UnknownHostException)
                        SnackBarUtils.showSnackBar(this@PurchaseReturnFinishActivity, getString(R.string.error_message_connect_error))
                    else
                        SnackBarUtils.showSnackBar(this@PurchaseReturnFinishActivity, getString(R.string.error_message_went_wront))
                }
            }
                ...
                try {
                    ...
                    if (result) {
                        ...
                        runOnUiThread {
                            ...
                            SnackBarUtils.showSnackBar(this@PurchaseReturnFinishActivity, message)
                        }
                    } else {
                        val message = jo.getString("Message")
                        runOnUiThread {
                    ...
                    runOnUiThread {
                        pbLoadingSaveAsPDF.visibility = View.GONE
                        fabSaveAsPDF.isEnabled = true
                        SnackBarUtils.showSnackBar(this@PurchaseReturnFinishActivity, "$resp")
                    }
                }
            }
        })
    ...
    private fun uploadData(isUpdate: Boolean,
        ...
        joSalesSummery.put("Purchase_RuturnDate", date)
        ...
        Log.e("_______________req", "save invoice reference number:$referenceNo")
        ...
        if (isUpdate) {
            body.add("PurchaseReturnID", invoiceId)
        }

        val url = "https://api.myjson.com/bins/1elzb0"
        val request = Request.Builder()
                .url(if (isUpdate) PublicUrls.URL_EDIT_PURCHASE_RETURN_INVOICE else PublicUrls.URL_SAVE_PURCHASE_RETURN_INVOICE)
                .post(body.build())
                .build()
                ...
                try {
                    ...
                    if (result) {
                        ...
                        runOnUiThread {
                            ...
                            if (!isUpdate)
                                DataCollections.getInstance(this@PurchaseReturnFinishActivity).addPurchaseReturnInvoice(ivId, ivNo, TypeManager.INVOICE_TYPE_OLD)
                            SnackBarUtils.showSnackBar(this@PurchaseReturnFinishActivity, message)
    ...
    private val bt = BluetoothUtils(object : BluetoothUtils.BluetoothListener {
        override fun onSuccess(isOn: Boolean) {
            if (isOn) {
                dbp.discover(this@PurchaseReturnFinishActivity)
            }
        }
        ...
        override fun onError() {
            ...
            SnackBarUtils.showSnackBar(this@PurchaseReturnFinishActivity, "Cant turn on bluetooth")
        }
    })
    ...
    private val dbp = BluetoothPrinterDiscovery(object : BluetoothPrinterDiscovery.PrinterDiscoverListener {
        override fun onSuccess(discoveredPrinter: DiscoveredPrinter) {
            ...
            try {
                ...
                if (zebraPrinter.currentStatus.isReadyToPrint || zebraPrinter.currentStatus.isPaused) {
                    val header = StringUtils.derialize(DataCollections.getInstance(this@PurchaseReturnFinishActivity).getUser()!!.header)
                    val footer = StringUtils.derialize(DataCollections.getInstance(this@PurchaseReturnFinishActivity).getUser()!!.footer)
                    val cash = ("%." + DataCollections.getInstance(this@PurchaseReturnFinishActivity).getUser()!!.amountType + "f").format(MathUtils.toDouble(tvValueCash.text.toString()))
                    val card = ("%." + DataCollections.getInstance(this@PurchaseReturnFinishActivity).getUser()!!.amountType + "f").format(MathUtils.toDouble(tvValueCredit.text.toString()))
                    val balanceAmt = tvValueBalance.text.toString()
                    val discountAmt =  ("%." + DataCollections.getInstance(this@PurchaseReturnFinishActivity).getUser()!!.amountType + "f").format(MathUtils.toDouble(tvValueDiscountAmount.text.toString()))
                    val amtType = DataCollections.getInstance(this@PurchaseReturnFinishActivity).getUser()!!.amountType
                ...
                fabPrint.isEnabled = true
                pbLoadingPrint.visibility = View.GONE
                SnackBarUtils.showSnackBar(this@PurchaseReturnFinishActivity, "Can't connect to printer")
            }
        }
        ...
        override fun onFail() {
            fabPrint.isEnabled = true
            pbLoadingPrint.visibility = View.GONE
            SnackBarUtils.showSnackBar(this@PurchaseReturnFinishActivity, "Printer not found")
        }
    })
    ...
    private val printResult = object : PrintTask.ZebraPrinterNotify {
        override fun notifyChange() {
            fabPrint.isEnabled = true
            pbLoadingPrint.visibility = View.GONE
            SnackBarUtils.showSnackBar(this@PurchaseReturnFinishActivity, "Print success")
        }
    }
                        ...
                        .setPositiveButton("YES") { p0, p1 ->
                            if (bt.isTurning() || dbp.isDiscovering() || (printTask != null && printTask!!.status == AsyncTask.Status.RUNNING)) {
                                AlertDialog.Builder(this)
                                        ...
                                        .setPositiveButton("YES") { p0, p1 ->
                                            isExit = true
                                            this@PurchaseReturnFinishActivity.onBackPressed()
                                            if (bt.isTurning())
                                                bt.cancelListener(this@PurchaseReturnFinishActivity)
                                            if (dbp.isDiscovering())
                                                dbp.cancelDiscovery()
